...
namespace APIDashboard.Models
{
    public class DashBoardPreProd
    {
        ...
        public List<CalendarsTitle> _CalendarsTitle;
        ...
        public DashBoardPreProd()
        {
            ...
            _CalendarsTitle = GetCalendarsTitleList();
        ...
        public List<Informations> GetImportantInformationsList()
        {
            ...
            try
            {
				string fileMethodsStatus = Path.Combine(Directory.GetCurrentDirectory(),"wwwroot/APISOAP/PreProduction","ImportantInformations.csv");
        ...
        public List<PieChart> GetPieChartList()
        {
            ...
            try
            {
				string fileMethodsStatus = Path.Combine(Directory.GetCurrentDirectory(),"wwwroot/APISOAP/PreProduction","PieChart.csv");
                using (var reader = new StreamReader(fileMethodsStatus))
                {
        ...
        public List<Issues> GetAPIIssuesList()
        {
            ...
            try
            {
				string fileMethodsStatus = Path.Combine(Directory.GetCurrentDirectory(),"wwwroot/APISOAP/PreProduction","APIIssues.csv");
                using (var reader = new StreamReader(fileMethodsStatus))
                {
        ...
        public List<Methods> GetMethodsStatusList()
        {
            ...
            try
            {
				string fileMethodsStatus = Path.Combine(Directory.GetCurrentDirectory(),"wwwroot/APISOAP/PreProduction","StatusByMethod.csv");
                using (var reader = new StreamReader(fileMethodsStatus))
                {
        ...
        public List<CalendarsTitle> GetCalendarsTitleList()
        {
            List<CalendarsTitle> CalendarsTitleList = new List<CalendarsTitle>();
            string error = string.Empty;

            try
            {
                string fileMethodsStatus = Path.Combine(Directory.GetCurrentDirectory(),"wwwroot/APISOAP/PreProduction","CalendarsTitle.csv");
                using (var reader = new StreamReader(fileMethodsStatus))
                {
                    List<string> listCalendarsTitle = new List<string>();
                    while (!reader.EndOfStream)
                    {
                        var line = reader.ReadLine();
                        var values = line.Split(';');

                        listCalendarsTitle.Add(values[0]);

                        string CalsTitle = "";
                        if (String.IsNullOrEmpty(CalsTitle))
                        {
                            CalsTitle = values[0];
                        }

                        CalendarsTitleList.Add(new CalendarsTitle()
                        {
                            CalendarsCardTitle = CalsTitle
                        });
                    }
                    reader.Close();
                }    
            ...
            return CalendarsTitleList;
        }

        public List<CalendarM1> GetCalendarsDateStateListM1()
        {
            List<CalendarM1> CalendarsDateStateListM1 = new List<CalendarM1>();
            string error = string.Empty;

            try
            {
				string fileMethodsStatus = Path.Combine(Directory.GetCurrentDirectory(),"wwwroot/APISOAP/PreProduction","CalendarM1.csv");
                using (var reader = new StreamReader(fileMethodsStatus))
                {
                    ...
                    while (!reader.EndOfStream)
                    {
                        ...
                        listMonthCalM1.Add(values[0]);
                        listDayOfTheWeekDayStatusOfTheDay1M1.Add(values[1]);
                        listDayOfTheWeekDayStatusOfTheDay2M1.Add(values[2]);
                        listDayOfTheWeekDayStatusOfTheDay3M1.Add(values[3]);
                        listDayOfTheWeekDayStatusOfTheDay4M1.Add(values[4]);
                        listDayOfTheWeekDayStatusOfTheDay5M1.Add(values[5]);

                        string MonthCaldrM1 = "";
                        if (String.IsNullOrEmpty(MonthCaldrM1))
                        {
                            MonthCaldrM1 = values[0];
                        }
                        ...
                        CalendarsDateStateListM1.Add(new CalendarM1()
                        {
        ...
        {
            List<CalendarM2> CalendarsDateStateListM2 = new List<CalendarM2>();
            string error = string.Empty;

            try
            {
				string fileMethodsStatus = Path.Combine(Directory.GetCurrentDirectory(),"wwwroot/APISOAP/PreProduction","CalendarM2.csv");
                using (var reader = new StreamReader(fileMethodsStatus))
                {
                    ...
                    while (!reader.EndOfStream)
                    {
                        ...
                        CalendarsDateStateListM2.Add(new CalendarM2()
                        {
            ...
            return CalendarsDateStateListM2;
        }
    }
}